...
## 简介
本文件系统性梳理小说转剧本工具的核心服务模块，覆盖从原始文本解析、章节切分、LLM 异步调用、转换引擎、连续性维护、剧本组装、数据校验到 YAML 导出的完整链路。文档重点解释：
- 文件处理服务的多格式支持与编码容错
...
## 项目结构
核心服务位于 app/services 下，模型与提示词分别位于 app/models 与 app/prompts，测试位于 tests。下图展示与本文相关的关键文件与模块：
...
- 转换引擎（converter）
  - 将章节转换为场景元素；通过“前一章摘要”维持连续性；对超长章节截断以控制令牌预算；失败时降级生成最小化场景。
- 剧本组装服务（assembler）
  - 合并各章节结果为完整剧本文本；全局重编号；补全 characters_present；设置角色首次出场。
- 数据验证服务（validator）
  - 结构完整性校验：元数据必填、编号连续、每幕至少一场、每场至少一个元素；角色引用一致性；错误/警告分级。
...
CS-->>CV : "章节列表含标题、内容、偏移"
loop 遍历每个章节
CV->>LLM : "发送系统提示 + 用户提示含角色目录、上文摘要、章节内容"
...
## 依赖关系分析
- 内聚与耦合
  - 各服务职责清晰：解析、切分、调用 LLM、转换、组装、校验、导出。
  - 耦合点主要在：
...
- 转换引擎
  - 截断策略避免超长输入；连续性摘要可复用上文以减少重复提示长度。建议对角色目录与章节标题做缓存。
- 组装与导出
...
## 结论
该核心服务模块以清晰的职责划分与稳健的错误处理实现了从多格式文本到结构化剧本的自动化转换。通过正则与启发式相结合的章节切分、异步 LLM 调用与连续性摘要、全局编号与角色出场补全、严格的数据验证与格式化 YAML 导出，形成了高可用的转换流水线。建议在生产环境中结合并发与缓存策略进一步优化吞吐与稳定性。
...
- 测试参考
  - 参考 tests/test_file_parser.py、tests/test_chapter_splitter.py、tests/test_assembler.py 获取典型用例与断言方式。