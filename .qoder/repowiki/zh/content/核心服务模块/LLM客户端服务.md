...
- [app/prompts/regeneration.py](file://app/prompts/regeneration.py)
...
- **新增**：异步流式响应处理：complete_stream()方法支持实时文本块生成
- 异步调用最佳实践与性能优化
- 错误码处理、速率限制应对与监控指标
- 与提示工程模块的集成与数据流
...
PROMPT_REGEN["app/prompts/regeneration.py"]
end
CFG["app/config.py"]
MAIN --> ROUTES
ROUTES --> LLM
...
- [app/prompts/regeneration.py:1-75](file://app/prompts/regeneration.py#L1-L75)
...
- 异步LLM客户端（DeepSeekClient）
  ...
  - **新增**：complete_stream()方法支持异步流式响应，实时生成文本块
  - 内置指数退避重试与异常记录
  - 提供关闭底层HTTP客户端的能力
- 配置中心（Settings）
...
章节来源
- [app/services/llm_client.py:18-155](file://app/services/llm_client.py#L18-L155)
...
- [app/services/converter.py:36-101](file://app/services/converter.py#L36-L101)
...
## 架构总览
下图展示了从API路由到LLM客户端，再到提示工程与模型解析的整体流程，包括新增的流式处理能力。
...
Note over API,LLM : 新增流式处理支持
Client->>API : POST /api/regenerate/{job_id}
API->>LLM : complete_stream(system,user)
LLM->>OpenAI : chat.completions.create(stream=True)
OpenAI-->>LLM : 文本块流
LLM-->>API : (full_text_coroutine, chunk_generator)
API-->>Client : 实时文本块流
```

图表来源
- [app/api/routes.py:280-378](file://app/api/routes.py#L280-L378)
- [app/services/character_extractor.py:21-76](file://app/services/character_extractor.py#L21-L76)
- [app/services/converter.py:36-101](file://app/services/converter.py#L36-L101)
- [app/services/llm_client.py:89-138](file://app/services/llm_client.py#L89-L138)
...
- 设计要点
  ...
  - 关闭方法用于释放底层连接资源
  - **新增**：complete_stream()方法支持异步流式响应处理
- 数据流与复杂度
  - 输入：系统提示、用户提示、可选的响应模型类型
  - 输出：原始文本或Pydantic模型实例，或（完整文本协程，文本块生成器）
...
+complete_stream(system_prompt, user_prompt, temperature) tuple[str, AsyncGenerator[str, None]]
...
### 流式响应处理（complete_stream方法）
- 设计要点
  - 返回元组：(StreamResult实例, AsyncGenerator[str, None])
  - StreamResult实例包含full_text属性，用于获取完整响应文本
  - AsyncGenerator提供实时文本块流，支持异步迭代
  - 内部使用AsyncOpenAI的流式API，逐块返回模型输出
  - 异常处理：捕获错误并返回错误信息，同时记录完整文本
- 使用模式
  - 通过异步for循环迭代文本块生成器
  - 迭代完成后，从StreamResult.full_text获取完整文本
  - 适用于实时显示、进度反馈、增量处理等场景
- 典型应用场景
  - YAML内容重新生成的实时流式显示
  - 剧本格式转换的实时预览
  - 用户交互式编辑的即时反馈
...
App->>LLM : complete_stream(system,user)
LLM->>OpenAI : chat.completions.create(stream=True)
OpenAI-->>LLM : 文本块1
LLM-->>App : yield 文本块1
App->>App : 处理文本块1
...
App->>App : 处理文本块N
App->>LLM : 获取完整文本
LLM-->>App : StreamResult.full_text
```

图表来源
- [app/services/llm_client.py:89-138](file://app/services/llm_client.py#L89-L138)
- [app/api/routes.py:284-350](file://app/api/routes.py#L284-L350)
...
- [app/services/character_extractor.py:21-76](file://app/services/character_extractor.py#L21-L76)
- [app/prompts/character_extraction.py:1-54](file://app/prompts/character_extraction.py#L1-L54)
- [app/services/llm_client.py:34-87](file://app/services/llm_client.py#L34-L87)
...
- **新增**：流式转换支持
  - 支持两种模式：传统JSON解析模式和流式文本块模式
  - 流式模式下，实时显示转换进度和中间结果
  - 通过stream_callback回调函数实现增量显示
- 容错与回退
  - 转换失败时生成最小化Act作为回退
  - 连续性摘要生成失败时回退到最后场景描述
- 与LLM客户端集成
...
B --> C["组装提示模板"]
C --> D{"选择模式"}
D --> |JSON模式| E["调用complete()"]
D --> |流式模式| F["调用complete_stream()"]
E --> G["解析JSON结构"]
F --> H["实时显示文本块"]
G --> I["解析Act/Scene/元素"]
H --> I
...
- [app/services/converter.py:36-101](file://app/services/converter.py#L36-L101)
- [app/prompts/screenplay_conversion.py:1-99](file://app/prompts/screenplay_conversion.py#L1-L99)
- [app/services/llm_client.py:34-138](file://app/services/llm_client.py#L34-L138)
...
- 作业存储
  ...
  - **新增**：支持流式缓冲区，用于实时文本块存储
- SSE状态流
  - 通过Server-Sent Events向客户端推送实时进度
  - **新增**：支持实时文本块流，用于流式处理场景
- 背景转换管线
  - 串行执行解析、拆分、角色提取、章节转换、组装、验证、导出
  - 每个阶段更新进度百分比与当前/总章节数
  - 最终关闭LLM客户端以释放连接
- **新增**：流式处理端点
  - /api/regenerate/{job_id}：基于用户建议重新生成YAML内容
  - /api/screenplay/{job_id}：将YAML转换为格式化剧本
  - 支持Server-Sent Events实时推送处理结果
...
API-->>Client : 推送进度
Client->>API : POST /api/regenerate/{job_id} (流式)
API->>LLM : complete_stream()
LLM-->>API : (full_text_coroutine, chunk_generator)
API-->>Client : 实时文本块流
Client->>API : 下载结果/预览
```
...
图表来源
- [app/api/routes.py:131-479](file://app/api/routes.py#L131-L479)
...
PROMPT_REGEN["regeneration.py"] --> ROUTES
...
- [app/api/routes.py:15-33](file://app/api/routes.py#L15-L33)
- [app/services/llm_client.py:8-12](file://app/services/llm_client.py#L8-L12)
...
- 常见错误与定位
  ...
  - **新增**：流式处理错误
    - 流式连接中断：检查网络稳定性，流式处理会自动返回错误标记
    - 文本块编码问题：确保客户端正确处理UTF-8编码
    - 完整文本获取：通过StreamResult.full_text属性获取完整响应
- 日志与监控
  - 客户端与服务层广泛使用日志记录，便于追踪失败点
  - 建议在网关或代理层增加请求计数、错误率与P95/P99延迟指标
  - **新增**：流式处理监控：跟踪文本块数量、传输速率、错误率
- 速率限制应对
  - 当遇到429/5xx时，利用内置重试；若持续失败，建议引入外部限流器与熔断策略
  - 对于批量转换，建议分批提交并加入队列与背压
  - **新增**：流式处理限流：控制文本块生成速率，避免客户端过载
...
## 结论
本项目的LLM客户端服务以AsyncOpenAI为基础，结合Pydantic模型与严格的提示工程，实现了从角色提取到章节转换的端到端异步流程。**新增的流式处理能力**进一步提升了用户体验，支持实时文本块生成、增量显示和交互式编辑。通过统一配置中心、指数退避重试与结构化输出，系统在可用性与数据质量之间取得了良好平衡。建议在生产环境中进一步完善监控、限流与弹性伸缩策略，以应对高峰流量与复杂场景。
...
章节来源
- [app/prompts/character_extraction.py:19-41](file://app/prompts/character_extraction.py#L19-L41)
- [app/prompts/screenplay_conversion.py:29-80](file://app/prompts/screenplay_conversion.py#L29-L80)
- [app/models/screenplay.py:161-167](file://app/models/screenplay.py#L161-L167)
...
- **新增**：流式处理提示
  - 重新生成提示：根据用户建议改进剧本YAML
  - 剧本格式化提示：将YAML转换为标准剧本格式
  - 支持实时流式显示，提升用户体验

章节来源
- [app/prompts/character_extraction.py:3-54](file://app/prompts/character_extraction.py#L3-L54)
- [app/prompts/screenplay_conversion.py:3-99](file://app/prompts/screenplay_conversion.py#L3-L99)
- [app/prompts/regeneration.py:5-75](file://app/prompts/regeneration.py#L5-L75)

### 流式处理最佳实践
- **使用场景选择**
  - 适合流式处理：长文本生成、实时预览、交互式编辑
  - 适合传统模式：结构化数据解析、批量处理
- **性能优化建议**
  - 控制文本块大小，平衡延迟与带宽
  - 实现客户端缓冲，提升渲染性能
