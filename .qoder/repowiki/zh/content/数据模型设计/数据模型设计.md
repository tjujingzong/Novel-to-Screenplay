...
## 更新摘要
**变更内容**
- 新增ElementImportance枚举，定义动作元素的重要性级别（key、standard、background）
- 更新ActionElement模型，使用ElementImportance枚举替代字符串字段
- 新增ConversionStage枚举中的EXTRACTING_CHARACTERS阶段
- 更新转换管道状态管理，支持角色提取阶段
- 增强Schema规范，添加ElementImportance枚举说明
...
**更新** 本次更新反映了数据模型的重要改进：新增ElementImportance枚举用于精确控制动作元素的重要性级别，以及新增ConversionStage枚举的extracting_characters阶段，完善了整个转换管道的状态管理。

## 项目结构
项目采用按功能分层的组织方式：
- app/models：Pydantic数据模型与枚举定义
- app/services：业务服务（验证、导出、转换等）
- app/api：API路由与转换流程管理
- docs：Schema规范文档
- tests：模型与验证测试用例
...
S3["converter.py<br/>转换服务"]
S4["character_extractor.py<br/>角色提取服务"]
end
subgraph "API与流程"
A1["routes.py<br/>转换流程管理"]
end
subgraph "文档与测试"
...
- [app/services/character_extractor.py:1-167](file://app/services/character_extractor.py#L1-L167)
- [app/api/routes.py:636-696](file://app/api/routes.py#L636-L696)
...
**更新** 新增了ElementImportance枚举，为动作元素提供更精确的重要性分类，支持key（关键）、standard（标准）、background（背景）三个级别。
...
ActionElement --> ElementImportance : "使用"
```

**图表来源**
- [app/models/screenplay.py:17-167](file://app/models/screenplay.py#L17-L167)
- [app/models/enums.py:65-82](file://app/models/enums.py#L65-L82)
...
### 剧本元素（Element）模型族
- 动作（ActionElement）：动作描述、重要性等级（使用ElementImportance枚举）
...
**更新** ActionElement现在使用ElementImportance枚举而非字符串字段，提供更严格的类型安全性和更好的IDE支持。
...
### 枚举类型（Enums）
- 角色类型、时间段、内外景、元素类型、转场类型、格式、**重要性等级（新增ElementImportance）**、**转换阶段（新增EXTRACTING_CHARACTERS）**
- 设计要点：通过枚举约束取值，提升Schema稳定性与工具互操作性

**更新** 新增ElementImportance和ConversionStage枚举，完善了数据模型的类型安全性。
...
### 转换流程管理
- 转换阶段：从UPLOADED到COMPLETE的完整流程，包括EXTRACTING_CHARACTERS阶段
- 进度跟踪：每个阶段都有明确的进度百分比和状态更新
- 错误处理：完整的错误状态管理和回滚机制

**更新** 新增EXTRACTING_CHARACTERS阶段，专门负责角色提取任务，提高了转换流程的模块化程度。

**章节来源**
- [app/api/routes.py:636-696](file://app/api/routes.py#L636-L696)
- [app/models/enums.py:72-82](file://app/models/enums.py#L72-L82)

## 依赖关系分析
- 模型层内部依赖：元素类型通过判别联合聚合到场景元素数组；场景依赖场景标题与元素；结构依赖幕与场景；根模型依赖元数据、角色与结构
- 服务层依赖：验证服务依赖请求模型与元素类型；YAML导出服务依赖根模型；转换服务依赖角色提取服务
- API层依赖：路由管理依赖所有枚举类型和转换服务
...
- 枚举优化：使用Python枚举替代字符串常量，提供编译时类型检查和运行时性能优化

**更新** 枚举类型的使用提升了代码的类型安全性，减少了运行时字符串比较的开销。
...
- 常见验证问题
  ...
  - **重要性级别无效：ActionElement的importance必须是key、standard或background之一**
...
IterateElements --> CheckImportance["检查动作元素重要性级别"]
CheckImportance --> CheckRefs["检查角色引用与在场角色"]
CheckRefs --> Done(["完成"])
ReportTitle --> Done
ReportNoActs --> Done
CheckEmptyScenes --> Done
CheckImportance --> Done
CheckRefs --> Done
```

**更新** 新增了动作元素重要性级别的验证步骤，确保ElementImportance枚举的正确使用。
...
## 结论
本项目以Pydantic为核心，构建了清晰、稳定且LLM友好的数据模型体系。通过三层Schema（metadata、characters、structure）与判别联合元素类型，实现了可编辑、可验证、可渲染的YAML剧本格式。**新增的ElementImportance枚举提供了更精确的动作元素重要性控制，而新增的ConversionStage枚举则完善了转换管道的状态管理。** 枚举类型与严格的验证规则确保了数据一致性与工具互操作性。文档与测试进一步保障了模型的正确性与可维护性。
...
- 可扩展性
  ...
  - **ElementImportance枚举支持未来扩展新的重要性级别**

**更新** 新增了ElementImportance枚举的可扩展性说明。
...
- **重要性级别 → structure.acts[].scenes[].elements[].importance（ActionElement专用）**
...
- 映射要点
  ...
  - elements：动作（含重要性级别）、对白（含角色ID与显示名）、副作用、转场、注释

**更新** 在动作元素映射中新增了重要性级别的说明。

**章节来源**
- [docs/YAML_SCHEMA.md:331-473](file://docs/YAML_SCHEMA.md#L331-L473)

### 转换流程与状态管理
- **转换阶段流程**：UPLOADED → PARSING → SPLITTING → EXTRACTING_CHARACTERS → CONVERTING → ASSEMBLING → VALIDATING → COMPLETE
- **进度跟踪**：每个阶段都有明确的进度百分比和状态更新
- **错误处理**：完整的错误状态管理和回滚机制
- **角色提取**：专门的EXTRACTING_CHARACTERS阶段负责从小说章节中提取角色信息

**更新** 新增了完整的转换流程说明，突出了EXTRACTING_CHARACTERS阶段的重要性。
