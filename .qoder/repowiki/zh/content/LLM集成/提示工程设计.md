...
- 提示模板集中于 app/prompts/，分别定义系统提示与用户模板。
...
graph TB
subgraph "提示模板"
P1["screenplay_conversion.py"]
P2["continuity.py"]
P3["character_extraction.py"]
P4["chapter_detection.py"]
end
subgraph "服务层"
S1["converter.py"]
...
- 剧本转换提示（screenplay_conversion）
  - 设计原则：强调“展示而非叙述”、现时态动作、忠实原文、场景标题规范、对白自然化、动作可视化。
...
BuildUser --> CallLLM["调用 LLM 完成结构化 JSON"]
CallLLM --> Parse["解析 Act/Scene/Elements<br/>并分配全局场景号"]
Parse --> Summary["生成两句话连续性摘要"]
...
- [app/prompts/screenplay_conversion.py:1-91](file://app/prompts/screenplay_conversion.py#L1-L91)
- [app/services/converter.py:36-84](file://app/services/converter.py#L36-L84)
- [app/models/screenplay.py:105-141](file://app/models/screenplay.py#L105-L141)
...
participant CL as "LLM 客户端"
CV->>CV : "收集本章若干场景摘要片段"
CV->>CL : "complete(system_prompt, user_prompt, temperature=0.2)"
CL-->>CV : "两句话摘要"
...
Sample --> CallLLM["调用 LLM 提取角色(JSON)"]
...
- [app/services/character_extractor.py:95-145](file://app/services/character_extractor.py#L95-L145)
...
- [app/services/chapter_splitter.py:66-96](file://app/services/chapter_splitter.py#L66-L96)
...
```mermaid
graph LR
ConvPrompts["screenplay_conversion.py"] --> Converter["converter.py"]
ContPrompts["continuity.py"] --> Converter
CharPrompts["character_extraction.py"] --> CharExtractor["character_extractor.py"]
ChDetPrompts["chapter_detection.py"] --> Splitter["chapter_splitter.py"]
...
章节来源
- [app/services/converter.py:53-56](file://app/services/converter.py#L53-L56)
...
## 结论
本项目的提示工程设计以“结构化输出 + 严格约束 + 上下文注入”为核心，结合 LLM 的强大泛化能力与 Pydantic 的强类型校验，实现了从章节切分、角色提取到剧本转换与组装的完整流水线。通过两句话连续性摘要与角色目录的去重合并，有效提升了跨章节一致性与角色建模质量。建议在实际部署中持续监控 Token 使用、优化提示模板的清晰度与稳定性，并建立自动化评估指标以衡量输出质量。
